...
`im-not-ai-hermes`는 [`epoko77-ai/im-not-ai`](https://github.com/epoko77-ai/im-not-ai)를 Hermes Agent에서 사용할 수 있도록 옮긴 한국어 글쓰기 품질 개선 스킬이다.

이 저장소는 `andrea9292`가 관리하는 개인 downstream 포트다. 원본 프로젝트의 공식 배포판이 아니며, 특정 기관이나 연구소의 공식 스킬로 제시하지 않는다.

## 무엇을 하는 스킬인가

`humanize-korean`은 AI 생성문, 번역투, 기계번역 후편집투, 과도하게 정돈된 문장 구조, 반복적인 연결어와 결론 표현처럼 한국어 글에서 기계적으로 느껴지는 신호를 줄이기 위한 Hermes 스킬이다.

목표는 저자를 지우거나 출처를 속이는 것이 아니다. 의미, 사실, 장르, 문체 수준, 인용, 수치, 고유명사, 기술 용어를 보존하면서 한국어 문장의 리듬과 표현을 더 자연스럽게 다듬는 것이 목표다.

이 스킬은 저작권, 학업, 업무, 출판, 연구 윤리상 AI 사용 공개가 필요한 상황에서 저자성이나 작성 과정을 숨기는 용도로 사용해서는 안 된다.
...
한국어 AI 문체는 영어권 humanizer가 다루는 일반적인 과잉 수사와 다르게, 영한 번역투와 한국어 문장 리듬 문제로 드러나는 경우가 많다.
...
이 스킬은 이런 표현을 무조건 삭제하지 않는다. 장르와 맥락에 맞을 때는 보존하고, 반복되거나 번역투로 읽히는 경우에만 수술적으로 다듬는다.
...
이 저장소는 Hermes tap source로 쓰기 쉽도록 `skills/` 아래에 스킬 패키지를 둔다.
...
권장 설치 방식은 Hermes tap을 등록한 뒤 설치하는 것이다.
...
설치 후 현재 Hermes 세션에 바로 반영하려면 새 세션을 시작하거나 다음 명령을 실행한다.
...
Raw `SKILL.md` URL 설치는 권장하지 않는다. 이 스킬은 `references/`와 `scripts/` 파일을 함께 사용하므로, 단일 파일 설치 방식에서는 필요한 참고 파일이 빠질 수 있다.

## 사용법

Hermes에서 자연어로 요청하면 된다.

```text
humanize-korean으로 이 글의 번역투와 AI 문체를 줄여줘:
...
기본은 빠른 경로다. 짧거나 중간 길이의 글에서는 `quick-rules.md`와 핵심 분류 체계를 중심으로 읽고, 의미 보존을 우선하면서 문장 단위로 고친다.
...
Hermes에서는 Codex 포트처럼 고정된 5-role runtime을 그대로 실행하지 않는다. 대신 `SKILL.md`에 적힌 Hermes workflow를 따르고, 필요하면 `delegate_task` 같은 Hermes 도구를 사용해 탐지·윤문·의미 보존 검토를 분리할 수 있다.

## 산출물 기대값

인라인 텍스트를 다듬을 때는 보통 다음을 반환한다.
...
정량 지표는 긴 글이나 파일 기반 검토에서 보조 신호로 사용할 수 있다. 표준 라이브러리만 사용한다.
...
이 Hermes 포트는 taxonomy, quick rules, rewriting playbook, metrics, scholarship reference를 보존하되, Claude Code 전용 실행 개념을 Hermes 스킬 구조로 바꾼다.
...
이 저장소는 범용 한국어 글쓰기 품질 개선 스킬로 유지한다. 다음을 넣지 않는다.
...
개인 문체 재현이 필요하면 별도 private skill, style guide, voice sample을 이 스킬 위에 조합한다.

## 개인정보와 처리 범위

이 저장소 자체는 Hermes skill instruction, Markdown reference, Python helper script만 포함한다. 별도 서버, telemetry endpoint, background service, 외부 API connector를 추가하지 않는다.

다만 실제 텍스트 처리는 사용자가 실행하는 Hermes Agent와 선택한 LLM provider 설정을 따른다. 민감한 문서를 넣기 전에는 현재 Hermes profile, provider, logging, memory 설정을 확인해야 한다.
