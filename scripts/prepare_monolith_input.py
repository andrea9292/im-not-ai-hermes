#!/usr/bin/env python3
"""Humanize KR v1.6 — fast path input shim.

Pre-processes user input by computing v1.6 quantitative metrics and
prepending the result to the text the Hermes fast path reads. The fast path
...
Outputs (in {run_dir}):
  ...
  01_input_with_metrics.txt   — combined file the fast path Reads
...
Hard rules:
  ...
  - on metrics failure, write the combined file *without* the score block
    so the fast path degrades to v1.5 behaviour automatically.
"""
...
# Resolve Hermes skill layout. This file lives at:
#   scripts/prepare_monolith_input.py
# metrics.py is at:
#   references/metrics.py
# Run workspaces are created relative to the caller's cwd, not inside the skill.
...
# v1.6 metrics fallback. graceful degrade로 fast path 동작은 항상 보장.
try:
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="Humanize KR v1.6 fast path input shim")
