...
- Hermes tap-friendly repository 구조를 적용했다.
...
writer profile에는 다음 방식으로 설치 검증했다.
...
- 이 저장소는 Hermes skill package이며 웹 서비스 구현을 포함하지 않는다.
- `metrics.py`와 `metrics_v2.py`는 보조 신호 도구다. AI 여부를 판정하는 검출기가 아니다.
- raw URL 설치는 reference/script 파일을 누락할 수 있으므로 권장하지 않는다.
- 현재는 GitHub Actions CI를 포함하지 않는다. 릴리즈 전 로컬 smoke test를 수행한다.
