...
- **A-2** [S1] "~를 통해/통하여" 남발 → "~로", "~해서", "~함으로써"로 분산
...
- **E-1** 문장 길이 균일(모두 30~50자 부근) → 단문 1~2개와 장문 1개를 문단마다 의도적 삽입
...
- **I-1** "~한 것이다/~일 것이다" 종결 남발 → 확정 서술 "~다"로
...
2. **변경률**: 30% 이하인가. 확정 판정은 파일 윤문 직후 `verify_change_rate.py`로 계산한다 — 자가 산출값은 참고용
...
- **C**: S1 잔존 1~2 또는 자체검증 4항 이하 통과 — 사용자에게 heavy 경로 권고
- **D**: S1 잔존 3+ 또는 변경률 50% 이상 — 작업 중단 권고
