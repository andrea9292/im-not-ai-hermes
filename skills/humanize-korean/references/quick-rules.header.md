# Quick Rules — Hermes Route-Aware Core Rules (v2.2)
...
Hermes Agent가 한 흐름에서 탐지·윤문·자체검증을 끝내기 위해 사용하는 슬림 룰북. 본진 `ai-tell-taxonomy.md`에서 `quick: true` 패턴만 처방과 함께 한 줄로 압축해 **자동 생성**한다.
...
**구조 수렴 가드:** 변경률 30% 이상 = 경고, 50% 이상 = 강제 중단·롤백. 여기에 S1 목표 달성·C-8 전멸·golden/수치 주입 검사를 결합한다. 판정은 자가 산출이 아니라 `scripts/verify_gates.py`로 파일 윤문 직후 계산한다.
