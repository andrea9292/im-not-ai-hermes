...
1. 사실·주장·수치·날짜·고유명사·직접 인용·URL·법률·기술 식별자와 주장의 뼈대인 핵심 내용 명사·개념어를 보존한다.
2. 제목의 독립 줄, 표, 코드 펜스, 인라인 코드, 각주 번호·정의·앵커를 보존한다. 목록의 정보와 순서는 보존하되, 진단된 C-2/C-9에 해당하는 기계적 목록은 장르가 허용할 때만 손실 없이 산문으로 통합할 수 있다. 사용자 보존 지시가 있으면 목록 구조도 그대로 둔다.
...
3. 패턴을 찾기 전에 문장별 주어·목적어·보어에서 원문의 주장을 구성하는 핵심 내용 명사·개념어를 `anchor_ledger`로 잡고, 조사·어미를 제외한 원형 어휘를 기록한다.
4. 지배 패턴을 겨냥해 문단 단위로 고친다. D → A → I → G → H → F → B → C·J → E 순서는 참고하되 문맥을 우선한다. 관용구·추상 표현을 덜어낼 때는 수식어·형식명사만 제거하고 `anchor_ledger`의 어휘를 삭제하거나 동의어로 바꾸지 않는다.
5. 각 수정의 before/after와 taxonomy ID를 내부적으로 추적한다. 앵커가 사라지는 edit은 즉시 롤백한다.
6. 고유명사·수치·인용·구조·레지스터·새 주장 주입 여부와 `anchor_ledger`의 원형 어휘가 각각 최소 한 번 남았는지 자체검증한다.
7. 비문, 의미 드리프트, 앵커 유실이 생긴 수정은 원문 의미로 롤백한다. 자체 재시도는 한 번만 허용한다.
8. `output_path`에 쓴다.
