# AI 한글 티 분류 체계 v2.2 (Korean AI-Tell Taxonomy)

LLM(ChatGPT·Claude·Gemini 등)이 생성한 한글 글에서 반복적으로 관찰되는 "AI 티" 패턴을 10개 대분류 × 서브 패턴으로 정리한다. Hermes의 `light`·`standard`·`heavy` 경로가 공유하는 단일 진실 원천(SSOT)이다. 각 패턴마다 (1) 정의, (2) 시그니처 예문, (3) 심각도(S1 결정적 / S2 강함 / S3 약함), (4) 윤문 처방을 제공한다.
...
- 처방: "~로", "~해서", "~함으로써" 등으로 분산.
- _quick: true · quick_pattern: "~를 통해/통하여" 남발 · quick_fix: "~로", "~해서", "~함으로써"로 분산_
...
- _quick: true · quick_pattern: 문장 길이 균일(모두 30~50자 부근) · quick_fix: 단문 1~2개와 장문 1개를 문단마다 의도적 삽입_
...
- 처방: 확정 서술로. "~다"로 끝맺음.
- _quick: true · quick_pattern: "~한 것이다/~일 것이다" 종결 남발 · quick_fix: 확정 서술 "~다"로_
