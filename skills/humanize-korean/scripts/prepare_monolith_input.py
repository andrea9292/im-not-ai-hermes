#!/usr/bin/env python3
"""Humanize Korean v2.2 Hermes input shim.

Pre-processes user input by computing quantitative metrics (v1.6 8지표 +
v2.0 카운트형 지표) and prepending the result to the text the Hermes agent
reads. The combined file keeps metrics and source text together without
requiring Claude Code agents or slash commands.
...
Outputs (in {run_dir}):
  ...
  01_input_with_metrics.txt   — combined file the Hermes agent reads
...
Hard rules:
  ...
  - on metrics failure, write the combined file *without* the score block
    so the Hermes workflow can continue without metrics.
"""
