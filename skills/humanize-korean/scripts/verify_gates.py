...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument(
        "--stamp-summary",
        action="store_true",
        help="통합 게이트 결과를 --after 파일의 HUMANIZE-SUMMARY에 기록",
    )
    p.add_argument(
        "--execution-state",
        help="통합 게이트 결과를 기록할 00_execution.json 경로",
    )
    ...
    try:
        if args.stamp_summary:
            stamp_summary(args.after, rate * 100, code, scope_key)
        if args.execution_state:
            stamp_execution_state(args.execution_state, rate * 100, code, scope_key)
