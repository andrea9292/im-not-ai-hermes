...
원본 v2.0.0 계열은 v1.6.1의 fast monolith 흐름과 `final.md` 메타데이터 정책을 유지하면서, 한국 번역학계 기반 번역투 유형, post-editese metric layer, v2 baseline, scholarship reference를 추가한다.
...
3. Hermes 경로에 맞게 script import와 file path를 조정한다.
4. `python3 -m pytest skills/humanize-korean/tests -q`를 실행한다.
5. `hermes skills inspect andrea9292/im-not-ai-hermes/skills/humanize-korean`로 설치 가능성을 확인한다.
6. README, RELEASE_NOTES, 이 문서를 함께 갱신한다.
